...
void encodeSenML(String resource, float value, String unit)
{
    ...
    // I have to juggle the jsonResult buffer this way otherwise publish(...) will publish a blank string (I spent sooooo much unnecessary time on this...)
    // Not enough memory?
...
void setup()
{
    ...
    mqttClient.begin(F("test.mosquitto.org"), 1883);
    mqttClient.subscribe(BASE_TOPIC + String("led"), mqttLedSubscribeCallback);
...
void loop()
{
    ...
    if (now >= previousPublishTime + PUBLISH_PERIOD_LENGTH)
    {
        ...
        mqttClient.publish(BASE_TOPIC + String("temperature"), jsonResult);
